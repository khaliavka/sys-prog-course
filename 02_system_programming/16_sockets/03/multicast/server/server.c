...
int main(void)
{
    ...
    struct sockaddr_in multicast_endp;
    int fd;
    ...
    while (cancel_flag == 0)
    {
        ...
        if (sendto(fd, (const char *)timebuf, sizeof(timebuf), 0,
                   (const struct sockaddr *)&multicast_endp, (socklen_t)sizeof(multicast_endp)) != sizeof(timebuf))
            err_exit("sendto");
        sleep(SLEEPTMSEC);
    }
    const char command[] = EXITCMD;
    if (sendto(fd, command, sizeof(command), 0,
                   (const struct sockaddr *)&multicast_endp, (socklen_t)sizeof(multicast_endp)) != sizeof(command))
            err_exit("sendto");
